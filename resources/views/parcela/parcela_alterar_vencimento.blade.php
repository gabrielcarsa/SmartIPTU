...
<div class="card">
    ...
    <div class="card-body">
        <form class="row g-3"
            action="{{ isset($parcelaReceberOutros) ? '/contas_receber/definir_data_vencimento/' . Auth::user()->id : (isset($parcelaPagarOutros) ? '/contas_pagar/definir_data_vencimento/'. Auth::user()->id : '/parcela/definir_data_vencimento/' . Auth::user()->id . '?origem=' . request()->input('origem').'&lote_id=' . request()->input('lote_id')) }}"
